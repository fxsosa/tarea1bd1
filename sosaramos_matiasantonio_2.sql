...
--emplado, ya sea un cambio de departamento o un cambio de puesto de trabajo. Es decir, después de cada
--actualización de los atributos id_trabajo y/o id_departamento de la tabla empleados. Se debe gestionar
--adecuadamente las excepciones que ocurran en los subprogramas.
CREATE OR REPLACE TRIGGER trigger_historial_jobs
AFTER UPDATE OF ID_TRABAJO, ID_DEPARTAMENTO ON EMPLEADOS
FOR EACH ROW
BEGIN
	INSERT INTO HISTORIAL_TRABAJO(:OLD.EMPLEADO_ID, :OLD.FECHA_CONTRATO, SYSDATE, :OLD.TRABAJO_ID, :OLD.DEPARTAMENTO_ID);
END trigger_historial_jobs;
/
...
SELECT H.ID_EMPLEADO, E.NOMBRE||''||E.APELLIDO, H.FECHA_INICIO, H.FECHA_FIN, D.DEPARTAMENTO_NOMBRE, T.TITULO_TRABAJO
FROM HISTORIAL_TRABAJO H, EMPLEADO E
LEFT JOIN TRABAJOS T
